...
r.get('/lookup/:base', async (req, res) => {
  const row = await get('SELECT slug_id FROM pages WHERE slug_base=? ORDER BY updated_at DESC LIMIT 1', [req.params.base]);
...
r.post('/new', requireAdmin, async (req, res) => {
  ...
  const result = await run('INSERT INTO pages(slug_base, slug_id, title, content) VALUES(?,?,?,?)',[base, slug_id, title, content]);
  await upsertTags(result.lastID, tags);
  await sendAdminEvent('Page created', { user: req.session.user?.username, page: { title, slug_id, slug_base: base }, extra: { tags } });
  await sendFeedEvent('Nouvel article', { page: { title, slug_id }, author: req.session.user?.username, url: req.protocol + '://' + req.get('host') + '/wiki/' + slug_id, tags });
...
r.get('/wiki/:slugid', async (req, res) => {
  const ip = (req.headers['x-forwarded-for'] || '').split(',')[0].trim() || req.ip;
  const page = await get(`SELECT p.*,
    (SELECT COUNT(*) FROM likes WHERE page_id=p.id) AS likes,
    (SELECT COUNT(*) FROM likes WHERE page_id=p.id AND ip=?) AS userLiked
    FROM pages p WHERE slug_id=?`, [ip, req.params.slugid]);
  if (!page) return res.status(404).send('Page introuvable');
  await incrementView(page.id);
  const tlist = await all('SELECT name FROM tags t JOIN page_tags pt ON t.id=pt.tag_id WHERE pt.page_id=? ORDER BY name', [page.id]);
...
r.post('/wiki/:slugid/like', async (req, res) => {
  const ip = (req.headers['x-forwarded-for'] || '').split(',')[0].trim() || req.ip;
  const page = await get('SELECT id, slug_id, title, slug_base FROM pages WHERE slug_id=?', [req.params.slugid]);
  if (!page) return res.status(404).send('Page introuvable');
  const exists = await get('SELECT 1 FROM likes WHERE page_id=? AND ip=?', [page.id, ip]);
  if (exists) {
    await run('DELETE FROM likes WHERE page_id=? AND ip=?', [page.id, ip]);
    await sendAdminEvent('Like removed', { user: req.session.user?.username, page, extra: { ip } });
  } else {
    await run('INSERT INTO likes(page_id, ip) VALUES(?,?)', [page.id, ip]);
    await sendAdminEvent('Like added', { user: req.session.user?.username, page, extra: { ip } });
  }
...
r.post('/edit/:slugid', requireAdmin, async (req, res) => {
  ...
  if (!p) return res.status(404).send('Page introuvable');
  const base = slugify(title);
  await run('UPDATE pages SET title=?, content=?, slug_base=?, updated_at=CURRENT_TIMESTAMP WHERE slug_id=?', [title, content, base, req.params.slugid]);
  await run('DELETE FROM page_tags WHERE page_id=?', [p.id]);
  await upsertTags(p.id, tags);
  await sendAdminEvent('Page updated', { user: req.session.user?.username, page: { title, slug_id: req.params.slugid, slug_base: base }, extra: { tags } });
  res.redirect('/wiki/' + req.params.slugid);
});
r.delete('/delete/:slugid', requireAdmin, async (req, res) => {
  const p = await get('SELECT title, slug_id, slug_base FROM pages WHERE slug_id=?', [req.params.slugid]);
  await run('DELETE FROM pages WHERE slug_id=?', [req.params.slugid]);
  await sendAdminEvent('Page deleted', { user: req.session.user?.username, page: p });
  res.redirect('/');
});
...
async function upsertTags(pageId, csv='') {
  const names = csv.split(',').map(s=>s.trim()).filter(Boolean).map(s=>s.toLowerCase());
  for (const n of names) {
    await run('INSERT OR IGNORE INTO tags(name) VALUES(?)', [n]);
    const tag = await get('SELECT id FROM tags WHERE name=?', [n]);
    await run('INSERT OR IGNORE INTO page_tags(page_id, tag_id) VALUES(?,?)', [pageId, tag.id]);
  }
}
