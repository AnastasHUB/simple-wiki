...
app.use(async (req, res, next) => {
  ...
  const settings = await get('SELECT wiki_name AS wikiName, logo_url AS logoUrl, admin_webhook_url AS adminWebhook, feed_webhook_url AS feedWebhook, footer_text AS footerText FROM settings LIMIT 1');
...
app.get('/rss.xml', async (req, res) => {
  const rows = await (await import('./db.js')).all(`
